...
class Small_Corpus():
    ...
    def create_index(self):
        ...
        document_path_list = os.listdir(self.document_path)

        for filename in document_path_list:
            # open each document
            with open(os.path.join(self.document_path, filename), 'r', encoding='UTF-8') as f:
                lines = f.readlines()
    ...
    def calculation(self, query_name, query):
        ...
        self.score_dic[query_name] = dict(sorted(self.score_dic[query_name].items(), key=lambda x: x[1], reverse=True)[:round(860/len(query))])
    ...
    def calculate_recall(self, query_name):
        ...
        recall = rel / len(self.judgement_dic[query_name])
        return recall
    ...
    def calculate_p_10(self, query_name):
        ...
        for i in range(len(self.result_list[:10])):
            if self.result_list[i] in self.judgement_dic[query_name]:
                rel += 1
        # calcaulate P@10
    ...
    def calculate_r_percision(self, query_name):
        ...
        for i in range(len(self.result_list[:len(self.judgement_dic[query_name])])):
            if self.result_list[i] in self.judgement_dic[query_name]:
                rel += 1
        ...
        r_percision = rel / len(self.judgement_dic[query_name])
        return r_percision
    ...
    def calculate_ap(self, query_name):
        ...
        ap = p / len(self.judgement_dic[query_name])
        return ap
    ...
    def calculate_bpref(self, query_name):
        ...
        for document in self.result_list:
            ...
            if document in self.judgement_dic[query_name]:
                ...
                if non_rel <= len(self.judgement_dic[query_name]):
                    contri += 1 - non_rel / len(self.judgement_dic[query_name])
            else:
        ...
        bpref = contri / len(self.judgement_dic[query_name])
        return bpref
